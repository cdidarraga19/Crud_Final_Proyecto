...
public class ExportarPermisos {

	private XSSFWorkbook workbook;
	private XSSFSheet sheet;
	List<Permiso> permisoList;
	
	public ExportarPermisos (List<Permiso> listaPermiso) {
		
		this.permisoList = listaPermiso;
	...
	private void writeDataLines() {
		...
		for (Permiso usu : this.permisoList) {
			Row row = sheet.createRow(rowCount++);
			int columnCount = 0;
			createCell(row, columnCount++, usu.getIdPermiso(), style);
